...
describe("RewardsPool Testcases : ", function () {
    ...
    it("Should initialize LiquidityMiningManager values", async function () {
        ...
        await liquidityMiningManager.setRewardPerSecond("5000000000000000");
        let supply = await fossilToken.totalSupply();
    ...
    it("Should return rewards locked list", async function () {
        let rewardList = await rewardsPool.connect(user_2).getUserRewardsList(user_2.address);
        expect(rewardList.length).to.greaterThanOrEqual(1);
    });

    // --------------------------------- External Functions ----------------------------------------

    it("Should withdraw rewards after vesting period ends", async function () {
        const blockNumBefore = await ethers.provider.getBlockNumber();
        const blockBefore = await ethers.provider.getBlock(blockNumBefore);
        await ethers.provider.send("evm_mine", [blockBefore.timestamp+86400]);
        let rewards = await rewardsPool.connect(user_2).getUserRewardsList(user_2.address);
        let oldFossilBalance = await fossilToken.balanceOf(user_2.address);
        await rewardsPool.connect(user_2).withdraw(user_2.address, rewards.length-1);        
        let newFossilBalance = await fossilToken.balanceOf(user_2.address);
        expect(Number(newFossilBalance)).to.greaterThan(Number(oldFossilBalance));
    });

    it("Should change vesting period", async function () {
        let oldVestingPeriod = await rewardsPool.vestingPeriod();
        await rewardsPool.setVestingPeriod("1200");
        let newVestingPeriod = await rewardsPool.vestingPeriod();
        expect(Number(newVestingPeriod)).to.greaterThan(Number(oldVestingPeriod));
    });

    it("Should change contract owner", async function () {
        let newOwner = await rewardsPool.owner();
        await rewardsPool.changeContractOwner(user_2.address);
        let oldOwner = await rewardsPool.owner();
